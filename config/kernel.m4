...
AC_DEFUN([ZFS_LINUX_COMPILE], [
	...
	AC_ARG_VAR([KERNEL_ARCH], [Target architecture used with LLVM/CLANG toolchain])
	AC_TRY_COMMAND([
	    KBUILD_MODPOST_NOFINAL="$5" KBUILD_MODPOST_WARN="$6"
	    make modules -k -j$TEST_JOBS ${KERNEL_CC:+CC=$KERNEL_CC}
	    ${KERNEL_LD:+LD=$KERNEL_LD} ${KERNEL_LLVM:+LLVM=$KERNEL_LLVM} ${KERNEL_ARCH:+ARCH=$KERNEL_ARCH}
